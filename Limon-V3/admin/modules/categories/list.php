	...
	<div class="simplebox grid740">
		<div style="height: 32px;">
			...
			<div class="process float-right">
				...
				if($_GET["deleteID"]){
					$delete = mysql_query("DELETE FROM ".module_table." WHERE id='".$_GET["deleteID"]."'");
					$mth->delete('products',"WHERE category_id='".$_GET["deleteID"]."'",$unlink,1);
					if($delete){
						if(categoryDelete(module_table,$_GET["deleteID"],'files')){
							echo '<div class="success">Silme işlemi başarılı !</div>';
						}
					}
				}
				if($_POST["delete"]){
					...
					foreach($delete as $deleted){
						$delete = mysql_query("DELETE FROM ".module_table." WHERE id='".$deleted."'");
						if($delete){
							if(categoryDelete(module_table,$deleted,'files')){
								echo '<div class="success">Silme işlemi başarılı !</div>';
							}
						}
					}
        ...
        <table class="tablesorter list"> 
			...
			?>
				...
				<?php subCategoriesTable(module_table,$list["id"],1); ?> 
			<?php } ?>
            </tbody> 
        </table>
		</form>
